...
{#snippet chatIcon()}
    ...
    {#each chats as chat (chat.id)}
        <InfoStackItem onclick={() => onopenChat(chat.id)}>
            <div style="display: flex; flex-direction: column; gap: 0.25rem;">
                <h4 style="font-weight: 500; color: var(--color-dark); margin: 0;">{chat.title || 'Untitled Chat'}</h4>
                <span style="font-size: 0.75rem; color: var(--color-text-secondary);">{formatDate(chat.created_at)}</span>
            </div>
