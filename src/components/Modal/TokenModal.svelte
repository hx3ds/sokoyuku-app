<script>
    ...
    let {
        ...
        copyErrorMessage = 'Failed to copy token',
        onRefresh = undefined,
        onSave = undefined
    } = $props();
    ...
    function copyToken() {
        if (token) {
            navigator.clipboard.writeText(token).then(() => {
                showSuccess(t(copySuccessMessage));
            }).catch(() => {
                showError(t(copyErrorMessage));
            });
        }
    }
...
{#if show}
    ...
    >
        ...
        <Button variant="icon-button" onclick={copyToken} aria-label={t(copyButtonAriaLabel)} title={t('Copy to clipboard')}>
            ...
            <InfoStackItem>
                {#snippet actions()}
                    <div style="display: flex; gap: 0.5rem;">
                        {#if onRefresh}
                            <Button variant="text-button" onclick={onRefresh}>{t(refreshLabel)}</Button>
                        {/if}
                        {#if onSave}
                            <Button variant="text-button" onclick={handleSave} loading={saving}>
                                {t(saveLabel)}
                            </Button>
                        {/if}
                        <Button variant="text-button" onclick={close} aria-label={t(doneLabel)}>
                            {t(doneLabel)}
                        </Button>
                    </div>
                {/snippet}
        ...
        {:else}
            <div class="error-message">{t('Failed to load token')}</div>
        {/if}
    </Dialog>
{/if}
